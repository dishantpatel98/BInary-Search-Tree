...
ptr=root;
while(ptr!=NULL){                              //CODE TO FIND OUT THE LOCATION OF DESIRED KEY AND IT PARENT IN THE TREE
	if(key==ptr->data){
		break;
                          }
	par=ptr;
...
if(ptr->left!=NULL && ptr->right!=NULL){                //CASE WHERE THE NODE TO BE DELETED HAS TWO CHILDREN 
succ=ptr->right;
